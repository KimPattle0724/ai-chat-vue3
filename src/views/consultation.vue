        ...
        <div class="sidebar">
            <div class="ai-assistant-info">
                <div class="breathing-circle">
                    <img :src="iconUrl" alt="咨询助手" width="25" height="25" />
                </div>
                <h3 class="assistant-name">咨询助手</h3>
            ...
            <div class="emotion-garden">
                ...
                <div class="warm-tips">
                    <div class="emotion-status-text">
                        ...
                        <span class="status-emotion">{{currentEmotion?.isNegative ?? false ? '需要关注' : '正面'}}</span>
                    </div>
                    <div class="emotion-intensity">
                        <span class="intensity-dots">
                            <span v-for="dot in 3" :key="dot" class="dot" :class="{'active':getIntensityClass(currentEmotion?.emotionScore ?? 50) >= dot}"></span>
                        </span>
                        <span class="intensity-text">{{getRiskText(currentEmotion?.riskLevel ?? 0)}}</span>
                    </div>
            ...
            <div class="chat-header">
                <div class="header-left">
                    <div class="chat-avatar">
                        <img :src="iconUrl1" alt="咨询助手" width="30px" height="30px" />
                    </div>
                    <div class="chat-info">
                        <h2>咨询助手</h2>
                        <p>在线</p>
                ...
                <div v-for="msg in message" :key="msg.id" class="message-item" :class="msg.senderType === 1 ? 'user-message' : 'ai-message'">
                    ...
                    <div class="message-content">
                        ...
                        <div class="message-time">
                            {{msg.senderType === 2 && isAiTyping ? 'typinging...' : msg.createdAt}}
                        </div>
                    </div>
                </div>
                </div>               
                <div class="chat-input">
                    ...
                    <el-button 
                    :disabled="!userMessage.trim() || userMessage.trim().length > 500" 
                    type="primary" 
                    class="send-btn" 
                    @click="sendMessage">
                        <el-icon>
...
import { startSession,getSessionList,deleteSessionDetail,getSessionMessages,sendMessage,getSessionEmotion } from '@/api/frontend'
import { ElMessage } from 'element-plus'
import { ChatRound, DeleteFilled } from '@element-plus/icons-vue'
import MarkdownRenderer from '@/components/MarkdownRenderer.vue'
import { fetchEventSource } from '@microsoft/fetch-event-source'
import { isNameLocationCenter } from 'echarts/types/src/coord/axisHelper.js'
import { getCoordinateSystemDimensions } from 'echarts/types/src/echarts.all.js'
...
const createNewFrontendSession =  () => {
    const newSession = {
        sessionId:`temp_${Date.now()}`,// 会话ID，临时会话，过期时间为1小时
        status:'TEMP',// 会话状态，TEMP表示临时会话
        sessionTitle:'新对话'
    }
...
const sendMessage = () => {
    ...
    const inputMessage = userMessage.value.trim()
    userMessage.value = ''
    ...
    if(currentSession.value.status === 'TEMP'){
        startNewSession(inputMessage)
    } else {
        // 如果已有会话，调用发送消息的 API
        sendMessageToAI(currentSession.value.sessionId, inputMessage)
    }
}
...
const startNewSession = (message) => {
    ...
    startSession(sessionParams).then(res => {
        ...
        // 创建会话后获取消息列表
        getSessionMessages(res.sessionId).then(msgRes => {
            message.value = msgRes
            isAiTyping.value = false
            // 获取消息后再启动 AI 响应
            startAiResponce(currentSession.value.sessionId,message)
        }).catch(() => {
            isAiTyping.value = false
        })
        getSessionPage()
...
const startAiResponce = (sessionId,userMessage) => {
    if(isAiTyping.value){
        ElMessage.error('请稍后再试')
        return
    }
...
// 在已有会话中发送消息
const sendMessageToAI = (sessionId, userMessage) => {
    if(isAiTyping.value){
        ElMessage.error('请稍后再试')
        return
    }
    ...
    fetchEventSource(`/api/psychological-chat/stream`,{
        ...
        onmessage:(event) => {
            if(event.data){
                ...
                if(!raw) return
                const eventName = event.event
                const currentAiMessage = message.value[message.value.length - 1]
                if(eventName === 'done'){
                    isAiTyping.value = false
                    ctrl.abort()
                    loadSessionEmotion(currentSession.value.sessionId)
...
const handleError = (error) => {
    const aiMessage = message.value[message.value.length - 1]
    if(aiMessage){
        aiMessage.content = '发生错误，请稍后重试'
    }
    isAiTyping.value = false
    ElMessage.error('AI助手发生错误，请稍后重试')
}
...
const handleDeleteSession = (sessionId) => {
    deleteSessionDetail(sessionId).then(res => {
        ElMessage.success('删除成功')
        getSessionPage()// 刷新会话列表
    })
 }
