        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="情绪标签">
            <el-tag :type="getEmotionTagType(currentDetail.dominantEmotion)">
              {{ currentDetail.dominantEmotion || '无'  }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="睡眠质量">
            {{ currentDetail.sleepQuality || '无' }} / 5
          </el-descriptions-item>
          <el-descriptions-item label="压力等级">
            {{ currentDetail.stressLevel || '无' }} / 5
          </el-descriptions-item>
        </el-descriptions>
    </div>
    <div class="detail-section">
...
import { getEmotionalPages } from '@/api/admin'
...
const formItem = [
  ...
  {comp:'select',label:'情绪标签',prop:'moodScoreRange',placeholder:'请选择情绪标签',
  options:[
...
const handleSearch = async (formData) => {
  ...
  const { records,total } = await getEmotionalPages(params)
